...
public class PackageCompositeBuilder
{
    ...
    public static IPackageComponent CreatePallet(string palletId, List<Package> packages, int packagesPerBox = 3)
    {
        ...
        var boxNumber = 1;
        for (var i = 0; i < packages.Count; i += packagesPerBox)
        {
    ...
    public static IPackageComponent CreateBox(string boxId, List<Package> packages)
    {
        ...
        foreach (var package in packages) box.Add(new PackageLeaf(package));
    ...
    public static IPackageComponent CreateCrate(string crateId, string description, List<Package> packages)
    {
        ...
        foreach (var package in packages) crate.Add(new PackageLeaf(package));
    ...
    public static IPackageComponent CreateComplexHierarchy(string rootId, List<Package> packages)
    {
        ...
        var box3 = CreateBox($"{rootId}-CRATE-002-BOX-001",
            packages.Skip(midPoint).Take(packages.Count - midPoint).ToList());
        crate2.Add(box3);
    ...
    {
        var container =
            new PackageContainer(containerId, "Refrigerated Container (2°C - 8°C)", "Refrigerated Container");

        // Group by temperature requirements
        foreach (var package in packages.Where(p => p.RequiresRefrigeration)) container.Add(new PackageLeaf(package));
